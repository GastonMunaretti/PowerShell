...
{ 
    ...
    $User = Get-ADUser -Filter {surname -eq $APE -and name -eq  $NOM } -Properties EmployeeID, Pager, DisplayName

    if(($User.length -eq 0) -and ($data.Usuario -ne "")) 
    {
                  #Write-Output $nombre
         ...
         Write-Host ($data.Legajo,$data.Usuario, $data.ApellidoyNombre ,    "no hay registros en AD") -Separator "`t" 
    }
